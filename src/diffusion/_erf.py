...
from ._utils import interp_temperature, compute_diffusion


class ML_CVD_Model_Erf:
    def __init__(self):
    ...
    def __call__(self, filepath: str, dcal_type: int = 0, dx: float = 1e-4):
        ...
        def d_int_process(dep_t, d1, d1_ref, d1c, d2, d2_ref, d2c):
            ...
            for i in range(len(dep_t) - 1):
                dt_num = int((dep_t[-1] - dep_t[i]) / dt_i + 1)
                dti = (dep_t[-1] - dep_t[i]) / (dt_num - 1)
                time_int = np.linspace(dep_t[i], dep_t[-1], dt_num, endpoint=True)[:-1]
                d1_int_s_sum = 0.0
                d2_int_s_sum = 0.0
                for t_val in time_int:
                    t_c = interp_temperature(t_val, step_time, step_temperature) + 273.15
                    d1_int_s_sum += compute_diffusion(t_c, d1, d1c, d1_ref, dcal_type) * dti
                    d2_int_s_sum += compute_diffusion(t_c, d2, d2c, d2_ref, dcal_type) * dti
                D.append([d1_int_s_sum, d2_int_s_sum])
            return D

        D = d_int_process(dep_t, d1_coff, d1_temp_ref, d1_exp_c,
                          d2_coff, d2_temp_ref, d2_exp_c)
        self.D = D
        c1_res = np.array([])
        c2_res = np.array([])
        x_position = np.array([])
        ...
        for i in range(len(mesh)):
            ...
            x_position = np.hstack([x_position, mesh[i]])
